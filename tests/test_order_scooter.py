...
class TestOrderScooter:
    ...
    test_buttons = [StartPageLocators.upper_button_order, StartPageLocators.bottom_button_order]

    @allure.title('Проверка создания заказа через кнопки Заказать в шапке главной страницы и внизу страницы')
    @pytest.mark.parametrize('locator', test_buttons)
    def test_order_scooter_to_click_on_upper_button_order(self, driver, locator):

        order_page = OrderPage(driver)
        start_page = StartPage(driver)

        start_page.click_on_button_order(locator=locator)
